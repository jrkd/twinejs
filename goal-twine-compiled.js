console.log("inside goal-twine-compiled.");
...
__export(__webpack_require__(2)); //probably dont need to export this.
__export(__webpack_require__(1));
__export(__webpack_require__(3));
__export(__webpack_require__(12));
__export(__webpack_require__(4));
var planner_1 = __webpack_require__(14);
...
Object.defineProperty(exports, "__esModule", { value: true });
const binaryheap_1 = __webpack_require__(2);
class AStar {
    /**
...
Object.defineProperty(exports, "__esModule", { value: true });
const goalnode_1 = __webpack_require__(5);
const hash = __webpack_require__(6);
class GoalEdge {
    ...
    getNextNodeID() {
        return hash(this.action.effects.applyTo(this.prevNode.state));
...
const DataRetrieval_1 = __webpack_require__(11);
//import { RenderUpdatePage } from "./rendering/RenderUpdatePage";
const CurrentPlanExecution_1 = __webpack_require__(19);
class TwineDemo {
    static setup() {
        //TODO: Add event triggers
        /* setup all the actions setup in passages */
        TwineDemo.planData = DataRetrieval_1.DataRetrieval.loadDataFromPassages();
        window.test = TwineDemo.planData;
        window.planner = CurrentPlanExecution_1.CurrentPlanExecution;
        // $(document).one(':storyready', function (ev) {
        //     alert("STORY READY!");
        // });
        window.plans = CurrentPlanExecution_1.CurrentPlanExecution.runSearch(TwineDemo.planData.actionSets[0], TwineDemo.planData);
        // Whenever a passage is shown, that has been setup as an action
        // we apply that actions effects to the world, regardless of any current plans or actions' preconditions,
        // cause we're here now buddy, regardless. 
        //  
        // This will handle when a plan tells us to move to a new passage, 
        // as well as when a player does something to redirect the story to a passage. 
        $(document).on('shown.sm.passage', function (_event, eventObject) {
            const currentActionsetPlan = TwineDemo.currentActionsetPlan;
            let currentAction = TwineDemo.planData.actionSets[0].actions.find(action => action.name == eventObject.passage.name);
            if (currentAction) {
                TwineDemo.planData.currentWorldState = currentAction.effects.applyTo(TwineDemo.planData.currentWorldState);
                // This cant stay here forever, it only works if its part of the plan. 
                if (currentActionsetPlan && currentActionsetPlan.isCurrentActionApplied(TwineDemo.planData.currentWorldState)) {
                    currentActionsetPlan.currentActionPlan = currentActionsetPlan.currentActionPlan.slice(1);
                }
            }
            window.plans = CurrentPlanExecution_1.CurrentPlanExecution.runSearch(TwineDemo.planData.actionSets[0], TwineDemo.planData);
            if (window.plans && window.plans.length > 0) {
                window.plans.forEach(plan => {
                    if (plan.currentActionPlan[0]) {
                        let linkText = plan.currentActionPlan[0].action.name;
                        if (window.defaultLabelName) {
                            linkText = window.defaultLabelName;
                        }
                        $(".passage").append($("<a>")
                            .attr("data-goap-link", plan.currentGoalName)
                            .attr("href", "#next-goap-step")
                            .attr("style", "display:block")
                            .text(linkText).prop('outerHTML'));
                    }
                });
            }
        });
        // window.story.passages.forEach((passage: { source: string }) => {
        //     passage.source += "<a href='#next-goap-step' data-goap-link='data-goap-link'>Continue...</a>";
        // });
        $(document).on('click', 'a[data-passage]', (event) => {
            console.log("Player modified the worldstate");
            window.plans = CurrentPlanExecution_1.CurrentPlanExecution.runSearch(TwineDemo.planData.actionSets[0], TwineDemo.planData);
        });
        //
        $(document).on('click', '[data-goap-link]', (event) => {
            const currentActionsetPlan = window.plans.find(plan => plan.currentGoalName == $(event.currentTarget).attr('data-goap-link'));
            console.log("Next passage in the plan", currentActionsetPlan.currentActionPlan[0].action.name);
            let currentAction = currentActionsetPlan.currentActionPlan[0].action;
            if (currentActionsetPlan) {
                currentActionsetPlan.currentActionPlan = currentActionsetPlan.currentActionPlan.slice(1);
            }
            window.story.show(currentAction.name);
            event.preventDefault();
            return false;
        });
    }
}
exports.TwineDemo = TwineDemo;
...
Object.defineProperty(exports, "__esModule", { value: true });
const new_astar_1 = __webpack_require__(0);
//import * as $ from "jquery";
const ActionSet_1 = __webpack_require__(8);
const GlobalDataPlanning_1 = __webpack_require__(18);
// Data retrieval
class DataRetrieval {
    static loadDataFromPassages() {
        let result = new GlobalDataPlanning_1.GlobalDataPlanning();
        const $story = $("tw-storydata");
        const firstPassageID = $story.attr("startnode");
        const $firstPassage = $("tw-passagedata[pid='" + firstPassageID + "']");
        result.currentWorldState = new new_astar_1.WorldState();
        if ($firstPassage.attr('goappreconditions')) {
            result.currentWorldState = $.extend(result.currentWorldState, JSON.parse($firstPassage.attr('goappreconditions')));
        }
        if ($firstPassage.attr('goapeffects')) {
            result.goalStateJSON = JSON.parse($firstPassage.attr('goapeffects'));
        }
        let basicActionset = new ActionSet_1.ActionSet();
        basicActionset.name = "default";
        basicActionset.actions = [];
        $("tw-passagedata[pid!='" + firstPassageID + "']").each((_, item) => {
            const $passage = $(item);
            let action = new new_astar_1.NodeAction();
            action.name = $passage.attr("name");
            if ($passage.attr("goapcost")) {
                action.cost = Math.max(parseInt($passage.attr("goapcost")), 1);
            }
            if ($passage.attr("goappreconditions")) {
                action.preconditions = $.extend(new new_astar_1.WorldState(), JSON.parse($passage.attr("goappreconditions")));
            }
            if ($passage.attr("goapeffects")) {
                action.effects = $.extend(new new_astar_1.WorldState(), JSON.parse($passage.attr("goapeffects")));
            }
            basicActionset.actions.push(action);
        });
        window.defaultLabelName = $story.attr("goapdefaultlabel");
        result.actionSets = [basicActionset];
        return result;
    }
    static getJSONFromPassage(passage, marker_start = '/*START-ACTION', marker_end = 'END-ACTION*/') {
        if (passage.source.length == 0 || passage.source.indexOf(marker_start) == -1 || passage.source.indexOf(marker_end) == -1) {
            return "";
        }
        const strDefinition = passage.source.substring(passage.source.indexOf(marker_start) + marker_start.length, passage.source.indexOf(marker_end));
        return strDefinition;
    }
}
DataRetrieval.CurrentWorldStateKey = "initialStateJSON";
DataRetrieval.GoalWorldStateKey = "goalStateJSON";
DataRetrieval.ActionsetsKey = "actionsets";
...
Object.defineProperty(exports, "__esModule", { value: true });
const gridnode_1 = __webpack_require__(3);
const astar_1 = __webpack_require__(1);
const gridedge_1 = __webpack_require__(13);
...
Object.defineProperty(exports, "__esModule", { value: true });
class GridEdge {
    constructor(cost, prevNode, nextNode) {
        this.cost = cost;
        this.prevNode = prevNode;
        this.nextNode = nextNode;
    }
}
exports.GridEdge = GridEdge;
//# sourceMappingURL=gridedge.js.map
...
class Planner {
    ...
    preprocessGraph(startNode) {
        ...
        this.allNodesByID = {};
        this.allNodesByID[hash(startNode.state)] = startNode;
        while (nodesToProcess.length > 0) {
            ...
            currentNode.adjacentEdges.forEach(edge => {
                ...
                if (!this.allNodesByID.hasOwnProperty(nextNodeID)) {
                    this.allNodesByID[nextNodeID] = edge.generateNextNode();
                    nodesToProcess.push(this.allNodesByID[nextNodeID]);
                }
                edge.nextNode = this.allNodesByID[nextNodeID];
            });
        }
        return startNode;
    }
...
Object.defineProperty(exports, "__esModule", { value: true });
const new_astar_1 = __webpack_require__(0);
class GlobalDataPlanning {
    constructor() {
        this.currentWorldState = new new_astar_1.WorldState();
        this.currentPlans = new Map();
        this.actionSets = [];
        this.goalStateJSON = {};
        this.nextActionsetTurnIndex = 0;
        this.currentActivityIndex = 0;
    }
}
exports.GlobalDataPlanning = GlobalDataPlanning;
...
const new_astar_1 = __webpack_require__(0);
//import * as $ from "jquery";
const ActionSetPlan_1 = __webpack_require__(20);
const ActionSet_1 = __webpack_require__(8);
const hash = __webpack_require__(21);
//
// most of this class doesnt care about rendering, and could nearly be setup in the lib?
// definitely, at potentially at a generic level? Cause movement plans get fucked up to you know?
//
class CurrentPlanExecution {
    static addNewActionset(planData, includeEmptyAction = true) {
        let newActionSet = new ActionSet_1.ActionSet();
        let numNewActionsetsAlready = planData.actionSets.filter(actionset => actionset.name.startsWith("New-Actionset")).length;
        ...
        planData.actionSets.push(newActionSet);
        //RenderUpdatePage.renderActionsets(planData.actionSets);
        return planData;
    }
    static implementPlanWithAction(planData, actionset) {
        let plan = planData.currentPlans.get(actionset.name);
        if (!plan || !plan.isValid(planData.currentWorldState) || plan.isSuccessful(planData.currentWorldState)) {
            // This plan is no good. Look for a new one.
            if (!!plan && plan.isSuccessful(planData.currentWorldState)) {
                //RenderUpdatePage.logNewActivity("#activity-log-complete-goal-template", "", plan.currentGoalName, actionset.name);
            }
            planData.currentPlans.delete(actionset.name);
            let newPlan = CurrentPlanExecution.runSearch(actionset, planData);
            if (newPlan) {
                planData.currentPlans.set(actionset.name, newPlan[0]);
            }
            if (!newPlan) {
                //RenderUpdatePage.logNewActivity("#activity-log-no-goals-template", "", "", actionset.name);
            }
        }
        plan = planData.currentPlans.get(actionset.name); // Current plan might have changed.
        if (plan && plan.isValid(planData.currentWorldState)) {
            //Check for higher priority goals 
            let higherPriorityPlan = CurrentPlanExecution.runSearchForMoreImportantGoals(actionset, planData, plan.currentGoalName);
            if (higherPriorityPlan) {
                planData.currentPlans.delete(actionset.name);
                planData.currentPlans.set(actionset.name, higherPriorityPlan);
                plan = higherPriorityPlan;
            }
            //RenderUpdatePage.logNewActivity("#activity-log-use-action-template", plan.currentActionPlan[0].action.name, plan.currentGoalName, actionset.name);
            if (plan.isCurrentActionApplied(planData.currentWorldState)) {
                plan.currentActionPlan = plan.currentActionPlan.slice(1);
            }
            //planData.currentWorldState = plan.executeNextStep(planData.currentWorldState);
        }
        return planData;
    }
    static runSearchForMoreImportantGoals(actionset, planData, currentGoalName) {
        ...
        //setup current state
        let startState = planData.currentWorldState;
        let startNode = new new_astar_1.GoalNode();
        startNode.state = startState;
        let results = [];
        const goalNames = Object.keys(planData.goalStateJSON);
        let unmetGoals = [];
        let metGoalName = "";
        for (let index = 0; index < goalNames.length; ++index) {
            const goalName = goalNames[index];
            if (goalName == currentGoalName) {
                break;
            }
            let goalJSON = planData.goalStateJSON[goalName];
            let goalState = new new_astar_1.WorldState();
            $.extend(goalState, goalJSON);
            let goalResults = CurrentPlanExecution.runSearchForGoal(actionset.actions, startState, goalState);
            if (goalResults.length > 0) {
                results = goalResults;
                metGoalName = goalName;
                resultPlan = new ActionSetPlan_1.ActionSetPlan();
        ...
        //RenderUpdatePage.renderPlan(startNode, results, actionset, unmetGoals, metGoalName);
        return resultPlan;
    }
    static runSearch(actionset, planData) {
        let resultPlans = [];
        var $plannerResults = $("#plannerList");
        $plannerResults.empty();
        var planner = new new_astar_1.Planner();
        ...
        for (let index = 0; index < goalNames.length; ++index) {
            ...
            if (goalResults.length > 0) {
                results = goalResults;
                metGoalName = goalName;
                let resultPlan = new ActionSetPlan_1.ActionSetPlan();
                resultPlan.currentGoal = goalState;
                resultPlan.currentActionPlan = goalResults;
                resultPlan.currentGoalName = goalName;
                resultPlans.push(resultPlan);
            }
            else {
                unmetGoals.push(goalName);
            }
    ...
    static runSearchForGoal(_actions, startState, goalState) {
        let goalNode = new new_astar_1.GoalNode();
        goalNode.state = goalState;
        if (!CurrentPlanExecution.planner) {
            CurrentPlanExecution.planner = new new_astar_1.Planner();
            CurrentPlanExecution.planner.actions = _actions;
            CurrentPlanExecution.startNode = new new_astar_1.GoalNode();
            CurrentPlanExecution.startNode.state = startState;
            CurrentPlanExecution.planner.preprocessGraph(CurrentPlanExecution.startNode);
        }
        else {
            //We've already preprocessed, but the startstate is probably different to what it was.
            const startNodeID = hash(startState);
            if (CurrentPlanExecution.planner.allNodesByID.hasOwnProperty(startNodeID)) {
                CurrentPlanExecution.startNode = CurrentPlanExecution.planner.allNodesByID[startNodeID];
            }
            else {
                alert('We should in theory have every possible start node..? And not see this alert.');
            }
        }
        let results = new_astar_1.AStar.search(CurrentPlanExecution.planner, CurrentPlanExecution.startNode, goalNode, {});
        return results;
    }
}
CurrentPlanExecution.planner = null;
CurrentPlanExecution.startNode = null;
exports.CurrentPlanExecution = CurrentPlanExecution;
...
class ActionSetPlan {
    ...
    isCurrentActionApplied(currentWorldState) {
        return this.currentActionPlan[0].action.effects.containedWithin(currentWorldState);
    }
...
exports.ActionSetPlan = ActionSetPlan;
